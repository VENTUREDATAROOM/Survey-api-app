...
class EditFarmerDetailsController extends GetxController {
  ...
  final farmerIdTextController = TextEditingController().obs;
  ...
  void updateFarmerDeatilsApi() {
    loading.value = true;
    Map data = {
      "farmerId": int.parse(farmerIdTextController.value.text),
    ...
    _api.updateFarmerDetailsApi(data).then((value) {
      print(value);
      print("valuee");
      loading.value = false;
      if (value[AppConstants.requestCustomCode] == "200") {
        ...
        Utils.snackBar('Farmer Details', 'farmer details updated successfully');
      } else {
        Utils.snackBar(
          'Farmer Details',
  ...
  void resetForm() {
    farmerIdTextController.value.text = "";
